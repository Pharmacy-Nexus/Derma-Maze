...
(() => {
  ...
  const copy = {
    ar: {
      ...
      resetAllConfirm:'هل تريد مسح كل الإجابات والتقدم المحفوظ في فصل الطفيليات والبدء من الصفر؟ ستظل الأسئلة المحفوظة في المفضلة كما هي.',
    ...
    en: {
      ...
      resetAllConfirm:'Clear every answered question and saved progress in the Parasitic Infestation chapter and restart from zero? Bookmarks will stay saved.',
  ...
  function cacheElements(){
    [
      'parTopicJump','parTopicGrid','parModeSwitch','parTopicFilter','parTypeFilter','parDifficultyFilter','parSearch',
      'parApplyFilters','parRandomTen','parBookmarksOnly','parSessionTitle','parSessionCount','parScoreArc','parScorePercent',
      'parAnsweredCount','parCorrectCount','parSavedCount','parResetSession','parResetAllProgress','parProgressBar','parQuestionCard',
      'parQuestionId','parQuestionType','parQuestionDifficulty','parQuestionSource','parBookmarkBtn','parQuestionTopic',
      'parQuestionCounter','parQuestionImageWrap','parQuestionImage','parZoomImage','parQuestionText','parOptions',
      'parFeedback','parFeedbackIcon','parFeedbackTitle','parExplanation','parReasoning','parExamMessage',
      'parPrevQuestion','parSkipQuestion','parNextQuestion','parEmptyState','parExamFinish','parExamFinishTitle',
      'parFinishPercent','parFinishDetail','parFinishText','parReviewExam','parRestartExam','parOverallScore',
      'parCoverage','parStrongest','parReviewNext','parResultsGrid','parImageDialog','parCloseImage','parDialogImage',
  ...
  function setStaticTranslations(){
    document.querySelectorAll('[data-par-i18n]').forEach(el => {
      const value = t(el.dataset.parI18n);
      if(value !== undefined) el.textContent = value;
    });
    document.querySelectorAll('[data-par-i18n-html]').forEach(el => {
      const value = t(el.dataset.parI18nHtml);
      if(value !== undefined) el.innerHTML = value;
    });
    if(els.parSearch){
      els.parSearch.placeholder = lang()==='ar' ? 'الجرب، بيرمثرين، القمل، ليشمانيا...' : 'Scabies, permethrin, lice, leishmaniasis...';
    }
  }
  ...
  function renderTopics(){
    ...
    els.parTopicJump.querySelectorAll('[data-jump]').forEach(btn => {
      btn.addEventListener('click', () => {
        const target = document.getElementById(`topic-${btn.dataset.jump}`);
        target?.scrollIntoView({behavior:'smooth',block:'center'});
      });
    });
    els.parTopicGrid.querySelectorAll('[data-start-topic]').forEach(btn => {
  ...
  function renderFilterControls(){
    ...
    els.parTopicFilter.value = currentTopic;

    const types = ['MCQ','Case','Treatment','Image'];
    els.parTypeFilter.innerHTML =
      `<option value="all">${escapeHtml(t('allTypes'))}</option>` +
      types.map(type => `<option value="${type}">${escapeHtml(typeLabel(type))} (${window.PARASITIC_STATS.types[type]})</option>`).join('');
    els.parTypeFilter.value = currentType;

    const diffs = ['Easy','Medium','Hard'];
    els.parDifficultyFilter.innerHTML =
      `<option value="all">${escapeHtml(t('allDifficulties'))}</option>` +
      diffs.map(d => `<option value="${d}">${escapeHtml(difficultyLabel(d))} (${window.PARASITIC_STATS.difficulties[d]})</option>`).join('');
    els.parDifficultyFilter.value = currentDifficulty;

    els.parSearch.value = state.filters.search;
    updateModeButtons();
    els.parBookmarksOnly.classList.toggle('active',state.bookmarksOnly);
    const heart = els.parBookmarksOnly.querySelector('span');
    if(heart) heart.textContent = state.bookmarksOnly ? '♥' : '♡';
  }

  function updateModeButtons(){
    els.parModeSwitch.querySelectorAll('[data-mode]').forEach(btn => {
      btn.classList.toggle('active',btn.dataset.mode === state.mode);
    });
  }

  function readControls(){
    state.filters.topic = els.parTopicFilter.value;
    state.filters.type = els.parTypeFilter.value;
    state.filters.difficulty = els.parDifficultyFilter.value;
    state.filters.search = els.parSearch.value.trim();
  }
  ...
  function showPractice(){
    ...
    els.parEmptyState.hidden = !empty;
    els.parQuestionCard.hidden = empty;
    els.parExamFinish.hidden = true;
    if(empty && state.bookmarksOnly){
      const p = els.parEmptyState.querySelector('p');
  ...
  function renderQuestion(){
    ...
    els.parQuestionId.textContent = q.id;
    els.parQuestionType.textContent = typeLabel(q.type).toUpperCase();
    els.parQuestionDifficulty.textContent = difficultyLabel(q.difficulty).toUpperCase();
    els.parQuestionSource.textContent = q.source;
    els.parQuestionTopic.textContent = lang()==='ar' ? q.topicAr : q.topicEn.toUpperCase();
    els.parQuestionCounter.textContent = `${String(state.index+1).padStart(2,'0')} / ${String(state.session.length).padStart(2,'0')}`;
    els.parQuestionText.textContent = local(q.question);
    els.parProgressBar.style.width = `${((state.index+1)/state.session.length)*100}%`;

    els.parBookmarkBtn.classList.toggle('saved',state.bookmarks.has(q.id));
    els.parBookmarkBtn.textContent = state.bookmarks.has(q.id) ? '♥' : '♡';

    if(q.image){
      els.parQuestionImageWrap.hidden = false;
      els.parQuestionImage.src = q.image;
      els.parQuestionImage.alt = `${q.id} ${local(q.question)}`;
    }else{
      els.parQuestionImageWrap.hidden = true;
      els.parQuestionImage.removeAttribute('src');
    }

    els.parOptions.innerHTML = displayOptions.map((option,index) => `
      <button class="par-option" data-letter="${option.letter}">
        <span>${String.fromCharCode(65+index)}</span>
        <b>${escapeHtml(option[lang()] || option.en)}</b>
      </button>
    `).join('');

    els.parFeedback.className = 'par-feedback';
    els.parExamMessage.hidden = true;

    els.parOptions.querySelectorAll('.par-option').forEach(btn => {
    ...
    els.parPrevQuestion.disabled = state.index === 0;
    els.parNextQuestion.disabled = !answer && !state.reviewMode;
    els.parNextQuestion.querySelector('[data-par-i18n="question.next"]')?.replaceChildren(
  ...
  function chooseAnswer(letter){
    ...
    els.parNextQuestion.disabled = false;
    updateSessionSummary();
    renderResults();
  }
  ...
  function markExamSelection(letter){
    ...
    els.parExamMessage.hidden = false;
  }
  ...
  function revealAnswer(q,selectedLetter){
    ...
    els.parFeedback.className = `par-feedback show ${good ? 'good' : 'bad'}`;
    els.parFeedbackIcon.textContent = good ? '✓' : '×';
    els.parFeedbackTitle.textContent = good ? t('correct') : `${t('incorrect')} — ${t('correctAnswer')}: ${correctOptionText(q)}`;
    els.parExplanation.textContent = local(q.explanation);
    els.parReasoning.textContent = local(q.reasoning);
    els.parExamMessage.hidden = true;
  }
  ...
  function navigate(delta){
    ...
    state.index = next;
    renderQuestion();
    els.parQuestionCard.scrollIntoView({behavior:'smooth',block:'center'});
  }
  ...
  function finishSession(){
    ...
    els.parQuestionCard.hidden = true;
    els.parEmptyState.hidden = true;
    els.parExamFinish.hidden = false;
    els.parExamFinishTitle.textContent = state.mode === 'exam' ? t('examComplete') : t('studyComplete');
    els.parFinishPercent.textContent = `${result.percent}%`;
    els.parFinishDetail.textContent = `${result.correct} / ${result.answered} ${t('answeredWord')}`;
    els.parFinishText.textContent = state.mode === 'exam' ? t('examCompleteText') : t('studyCompleteText');
    updateSessionSummary();
    renderResults();
  }
  ...
  function reviewSession(){
    ...
    state.completed = false;
    els.parExamFinish.hidden = true;
    els.parQuestionCard.hidden = false;
    renderQuestion();
  }
  ...
  function updateSessionSummary(){
    ...
    els.parSessionTitle.textContent = sessionTitle();
    els.parSessionCount.textContent = `${state.session.length} ${t('questionsWord')}`;
    els.parAnsweredCount.textContent = s.answered;
    els.parCorrectCount.textContent = s.correct;
    els.parSavedCount.textContent = state.bookmarks.size;
    els.parScorePercent.textContent = `${s.percent}%`;
    els.parScoreArc.style.strokeDashoffset = String(314-(314*s.percent/100));
  }
  ...
  function renderResults(){
    ...
    els.parOverallScore.textContent = `${overall.percent}%`;
    els.parCoverage.textContent = `${overall.answered} / ${questions.length}`;
    els.parStrongest.textContent = strongest ? local(strongest.topic.title) : t('strongestEmpty');
    els.parReviewNext.textContent = review ? local(review.topic.title) : t('reviewEmpty');

    els.parResultsGrid.innerHTML = stats.map(item => `
      <article class="par-result-card" style="--pct:${item.percent}%">
  ...
  function openImage(){
    const q = currentQuestion();
    if(!q?.image) return;
    els.parDialogImage.src = q.image;
    els.parImageDialog.showModal();
  }
  ...
  function bindEvents(){
    ...
    els.parApplyFilters.addEventListener('click',() => {
      state.bookmarksOnly = false;
      startSession({kind:'filtered'});
      renderFilterControls();
    });
    els.parRandomTen.addEventListener('click',startRandomTen);
    ...
    els.parBookmarksOnly.addEventListener('click',toggleBookmarksOnly);
    els.parPrevQuestion.addEventListener('click',() => navigate(-1));
    els.parNextQuestion.addEventListener('click',() => navigate(1));
    els.parSkipQuestion.addEventListener('click',skipQuestion);
    els.parBookmarkBtn.addEventListener('click',toggleBookmark);
    els.parResetSession.addEventListener('click',resetCurrentSession);
    els.parResetAllProgress?.addEventListener('click',resetAllProgress);
    els.parReviewExam.addEventListener('click',reviewSession);
    els.parRestartExam.addEventListener('click',restartSession);
    els.parZoomImage.addEventListener('click',openImage);
    els.parQuestionImage.addEventListener('click',openImage);
    els.parCloseImage.addEventListener('click',() => els.parImageDialog.close());
    els.parImageDialog.addEventListener('click',e => { if(e.target === els.parImageDialog) els.parImageDialog.close(); });
    els.parSearch.addEventListener('keydown',e => {
      if(e.key === 'Enter'){
        e.preventDefault();
        state.bookmarksOnly = false;
        startSession({kind:'filtered'});
